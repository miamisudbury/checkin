...
if selectedOption == "X" or selectedOption == "x":
    ...
    searchfor = input("Enter Person ID or Name: ")
    if re.search("^[0-9]{5}$", searchfor) == None:
        for i in people:
            if (i["name"]) == searchfor:
                found = i
        ...
        try:
            ...
            print("The user you are trying to look for could not be found. Did you spell their name correctly?")
                
    if re.search("^[0-9]{5}$", searchfor) != None:
        for i in people:
            if (i["personId"]) == int(searchfor):
                found = i
                out = json.dumps(i, indent=2)
        try:
            print(out)
